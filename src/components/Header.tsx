...
const Header = () => {
    ...
    return (
        <header className="absolute left-0 right-0 top-0 bg-base text-dark py-0 px-12 h-[110px] z-50 lg:left-[7.5%] lg:right-[7.5%] lg:top-[2%] lg:rounded-t-[20px] lg:py-0 lg:px-12 md:left-0 md:right-0 md:top-0 md:rounded-0 md:p-0">
            <div className="container mx-auto h-full px-4 lg:px-6">
                <div className="flex items-center justify-between h-full">
                    ...
                    <nav className="hidden md:flex space-x-8 ml-8 items-center">
                        {navigationLinks.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className="text-dark hover:text-gray-700 transition-colors font-tajawal"
                            >
                                {item.name}
                            </Link>
                        ))}
                    ...
                    {/* Mobile menu button */}
                    <button
                        className="md:hidden text-dark transition-all duration-300"
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                    >
                        {isMenuOpen ? (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                className="h-6 w-6 transition-all duration-300 rotate-90">
            ...
            <div
                className={`md:hidden bg-base shadow-inner overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen
                ...
                <nav className="flex flex-col space-y-4 py-2">
                    {navigationLinks.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className="text-dark border-b-1 border-white hover:text-gray-700 py-2 font-tajawal text-lg text-center transition-colors duration-200"
